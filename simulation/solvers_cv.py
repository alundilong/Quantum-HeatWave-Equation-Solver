...
class Solver1DKrylov(Solver1D):
    """
    A subclass of Solver1D for solving with a Krylov solver.
    ...
    def run(self) -> Dict[str, Any]:
        """
        Runs Krylov solver and processes the results.

        Returns:
            Dict[str, Any]: A dictionary containing the field data and other results.
        """
        self.logger.info('Solving Krylov Dynamics.')
        initial_state = self.st.get_state(0)
        self.st.states = np.array(self.tf.simulate_krylov(psi0=initial_state,time_list=self.times))
        self.logger.info(f'Shape of st.states: {self.st.states.shape}')
        self.logger.info('Krylov System solved.')
...
class Solver1DKrylovQ(Solver1D):
    """
    A subclass of Solver1D for solving with a Krylov Quantum solver.
    ...
    def run(self) -> Dict[str, Any]:
        """
        Runs Krylov Quantum solver and processes the results.

        Returns:
            Dict[str, Any]: A dictionary containing the field data and other results.
        """
        self.logger.info('Solving KrylovQ Dynamics.')
        initial_state = self.st.get_state(0)
        self.st.states = np.array(self.tf.simulate_krylovQ(psi0=initial_state,time_list=self.times))
        self.logger.info(f'Shape of st.states: {self.st.states.shape}')
        self.logger.info('Krylov Quantum System solved.')
        self.logger.info(f'Time Length: {len(self.times)}')
...
class Solver1DDialation(Solver1D):
    """
    A subclass of Solver1D for solving with a classical
        Dialation solver.
    ...
    def run(self) -> Dict[str, Any]:
        """
        Runs Dialation solver and processes the results.

        Returns:
            Dict[str, Any]: A dictionary containing the field data and other results.
        """
        self.logger.info('Solving Dialation Dynamics.')
        initial_state = self.st.get_state(0)
        self.st.states = np.array(self.tf.simulate_dialation(psi0=initial_state,time_list=self.times))
        self.logger.info(f'Shape of st.states: {self.st.states.shape}')
        self.logger.info('Dialation System solved.')
        self.logger.info(f'Time Length: {len(self.times)}')
...
class Solver1DLindblad(Solver1D):
    ...
    def run(self) -> Dict[str, Any]:
        """
        Runs Lindblad master equation solver and processes the results.

        Returns:
            Dict[str, Any]: A dictionary containing the field data and other results.
        """
        self.logger.info('Solving Lindblad master equation.')
        initial_state = self.st.get_state(0)
        self.st.states = np.array(self.tf.simulate_lindblad(psi0=initial_state,time_list=self.times))
        self.logger.info(f'Shape of st.states: {self.st.states.shape}')
        self.logger.info('Lindblad master equation solved.')
        self.logger.info(f'Time Length: {len(self.times)}')
...
class Solver1DLCU(Solver1D):
    ...
    def run(self) -> Dict[str, Any]:
        """
        Runs LCU solver and processes the results.

        Returns:
            Dict[str, Any]: A dictionary containing the field data and other results.
        """
        self.logger.info('Solving LCU.')
        initial_state = self.st.get_state(0)
        self.st.states = np.array(self.tf.simulate_lcu(psi0=initial_state,time_list=self.times))
        self.logger.info(f'Shape of st.states: {self.st.states.shape}')
        self.logger.info('Lindblad master equation solved.')
        self.logger.info(f'Time Length: {len(self.times)}')
