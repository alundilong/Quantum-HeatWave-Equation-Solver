...
from .solvers_cv import Solver1DODE, \
        Solver1DEXP, \
        Solver1DEXP_emb,\
        Solver1DEXP_emb2, \
        Solver1DSplit,\
...
class ForwardExperiment1D:
    ...
    def add_solver(self, solver: str, dx: float, nx: int, dt: float,  nt: int, order: int,
        ...
        Args:
            solver (str): Solver type. One of 'ode', 'exp', 'exp_emb', 'exp_emb2', 'split',
            'splitQ', 'dialation', 'krylov', 'krylovQ', 'lindblad', 'warp', 'lcu', 'local', 'cloud'.
        ...
        # Check solver
        assert solver in ['ode', 'exp', 'exp_emb', 'exp_emb2', 'split', 'splitQ', 'warp', \
                'dialation', 'krylov', 'krylovQ', 'lindblad', \
                'lcu', 'local', 'cloud'], 'Solver not implemented.'
        ...
        match solver:
            ...
            case 'exp_emb2':
                self.solvers.append(Solver1DEXP_emb2(self.base_data, self.logger, **kwargs))
            case 'dialation':
            ...
            case 'splitQ':
                self.solvers.append(Solver1DSplitQ(self.base_data, self.logger, **kwargs))
            case 'krylov':
            ...
            case 'warp':
                self.solvers.append(Solver1DWarp(self.base_data, self.logger, **kwargs))
            case 'lcu':
                self.solvers.append(Solver1DLCU(self.base_data, self.logger, **kwargs))
            case 'local':
    ...
    def plot(self, mode, solvers=None, **kwargs):
        ...
        match mode:
            ...
            case 'medium':
                ...
                plot_medium(self.configs[solvers[0]]['alpha'],
                            self.configs[solvers[0]]['tau'], **kwargs)
            case 'initial':
